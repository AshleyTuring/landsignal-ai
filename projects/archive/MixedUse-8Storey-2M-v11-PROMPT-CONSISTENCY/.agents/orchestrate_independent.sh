...
CODEX_LIMIT_HIT=false   # Codex uses tools to read files (not stdin)
...
generate_gemini() {
    ...
    gemini -p "You are an expert property development analyst. Generate 1-3 unique property search strategies based on the requirements in the context above." \
...
generate_codex() {
    ...
    cat "$OUTPUT_DIR/shared_context.txt" | \
    codex exec --skip-git-repo-check "You are an expert property development analyst. Generate 1-3 unique property search strategies based on the requirements in the context above." \
...
generate_grok() {
    ...
    cat "$OUTPUT_DIR/shared_context.txt" | \
    opencode run -m opencode/grok-code "You are an expert property development analyst. Generate 1-3 unique property search strategies based on the requirements in the context above." \
...
generate_claude() {
    if [ "$CLAUDE_LIMIT_HIT" = true ]; then
        ...
        echo "🔵 Running Claude..."
        cat "$OUTPUT_DIR/shared_context.txt" | \
        claude -p "You are an expert property development analyst. Generate 1-3 unique property search strategies based on the requirements in the context above." \
