...
## Key Constraints
1. **Reference Materials:** See FILTER_PATTERNS and LEARNINGS_DATABASE sections below
2. **Technical Validity:** All filters MUST exist in available filters list (see below)
3. **Capital Feasibility:** Approaches MUST be close to budget requirements if specified (+15% over is allowed)
5. **Precedent Analysis:** MANDATORY - Generate at least ONE filter approach for EACH planning application provided in requirements (note there might be more than 1 filter approach for each planning application)
6. **Diversity:** Generate at least ONE approach using a completely different strategy (not precedent-based)
...
5. **Capital Feasibility:** Does approach meet budget requirements (+15% over is allowed)?
