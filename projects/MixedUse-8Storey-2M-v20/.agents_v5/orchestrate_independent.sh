...
# Determine iteration round (default to 1 for independent generation)
...
generate_gemini() {
    local system_prompt=$(get_system_prompt_for_round "gemini" "$ITERATION_ROUND")
    local lens_name=$(get_llm_lens_name "gemini" "$ITERATION_ROUND")
    
    echo "🔷 Running Gemini..."
    echo "   Lens: $lens_name"
    
    # Gemini doesn't support --system-prompt, so prepend to prompt
...
generate_codex() {
    local system_prompt=$(get_system_prompt_for_round "codex" "$ITERATION_ROUND")
    local lens_name=$(get_llm_lens_name "codex" "$ITERATION_ROUND")
    
    echo "🔶 Running ChatGPT..."
    echo "   Lens: $lens_name"
    
    # Codex doesn't read stdin - must give it file path
    codex exec --skip-git-repo-check "Read the file at $OUTPUT_DIR/shared_context.txt. $system_prompt. $PROMPT_GENERATION" \
    > "$OUTPUT_DIR/Codex_Approaches.md" 2>/dev/null
    
    echo "✅ ChatGPT complete"
}

generate_grok() {
    local system_prompt=$(get_system_prompt_for_round "grok" "$ITERATION_ROUND")
    local lens_name=$(get_llm_lens_name "grok" "$ITERATION_ROUND")
    
    echo "🟣 Running Grok..."
    echo "   Lens: $lens_name"
    
    # Grok (opencode) doesn't support --system-prompt, so prepend to prompt
    local full_prompt="$system_prompt
...
generate_claude() {
    if [ "$CLAUDE_LIMIT_HIT" = true ]; then
        ...
        local system_prompt=$(get_system_prompt_for_round "claude" "$ITERATION_ROUND")
        local lens_name=$(get_llm_lens_name "claude" "$ITERATION_ROUND")
...
echo "║  📐 Round: $ITERATION_ROUND                                           ║"
echo "║  📐 Mode: $(get_lens_mode_display $ITERATION_ROUND)"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
echo "Outputs:"
echo "  • Gemini ($(get_llm_lens_name gemini $ITERATION_ROUND)): $(wc -l < "$OUTPUT_DIR/Gemini_Approaches.md") lines"
echo "  • ChatGPT ($(get_llm_lens_name codex $ITERATION_ROUND)): $(wc -l < "$OUTPUT_DIR/Codex_Approaches.md") lines"
echo "  • Grok ($(get_llm_lens_name grok $ITERATION_ROUND)): $(wc -l < "$OUTPUT_DIR/Grok_Approaches.md") lines"
echo "  • Claude ($(get_llm_lens_name claude $ITERATION_ROUND)): $(wc -l < "$OUTPUT_DIR/Claude_Approaches.md") lines"
echo ""
